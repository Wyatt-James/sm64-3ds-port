...
typedef ShaderProgram ShaderProgram_t;
#endif

// Mandatory functions
bool                    gfx_rapi_z_is_from_0_to_1           (void);                                                                                // Returns true if the API's depth range is from 0 to 1.

uint32_t                gfx_rapi_new_texture                (void);                                                                                // Returns the index for a new texture. If no textures are available, it will return a valid texture, but which texture it is is implementation-defined.
...
void                    gfx_rapi_start_frame                (void);                                                                                // Called at the start of a frame.
void                    gfx_rapi_end_frame                  (void);                                                                                // Called at the end of a frame.
void                    gfx_rapi_finish_render              (void);                                                                                // Called after end_frame, but only if the frame was not dropped.

#if GFX_RAPI_COLOR_COMBINER == GFX_DISABLE
void                    gfx_rapi_unload_shader              (ShaderProgram_t *old_prg);                                          // Unloads the given shader.
void                    gfx_rapi_load_shader                (ShaderProgram_t *new_prg);                                          // Loads the given shader.
ShaderProgram_t*        gfx_rapi_create_and_load_new_shader (uint32_t shader_id);                                                // Creates and loads a new shader with the given ID. 
ShaderProgram_t*        gfx_rapi_lookup_shader              (uint32_t shader_id);                                                // Looks up an existing shader with the given ID.
void                    gfx_rapi_shader_get_info            (ShaderProgram_t *prg, uint8_t *num_inputs, bool used_textures[2]);  // Returns info about the currently loaded shader.
#endif
...
void                    gfx_rapi_apply_model_view_matrix    (void);                    // Sends the specified matrix from the current matrix set to the GPU.
void                    gfx_rapi_apply_projection_matrix    (void);                    // Sends the specified matrix from the current matrix set to the GPU.
